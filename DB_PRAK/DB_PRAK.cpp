...
<button onclick='window.history.back()' style = "margin-bottom: 10px;"> Atgal</button>
    ...
    CROW_ROUTE(app, "/login").methods("POST"_method)
        ...
        try {
            ...
            std::string db_host = env["DB_HOST"];
            std::string db_name = env["DB_NAME"];
            std::string db_user = env["DB_USER"];
            std::string db_pass = env["DB_PASSWORD"];

            sql::SQLString url(
                "jdbc:mariadb://" + db_host + "/" + db_name
            );
    ...
    CROW_ROUTE(app, "/search").methods("GET"_method, "POST"_method)
        ...
        if (req.method == crow::HTTPMethod::GET) {
            query = req.url_params.get("query")
                ? req.url_params.get("query")
                : "";
        }
        else {
            auto post_data = parseUrlEncoded(req.body);
